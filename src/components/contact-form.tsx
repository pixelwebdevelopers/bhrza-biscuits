...
export function ContactForm({ inverse = false }: { inverse?: boolean }) {
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!message || message.length > 1000)
      next.message = "Enter a message (up to 1,000 characters).";
    setErrors(next);
    ...
    <form onSubmit={submit} className="grid gap-4" noValidate>
      <div className="grid gap-4 sm:grid-cols-2">
        <Field label="Name" error={errors.name}>
          <Input name="name" maxLength={100} className={fieldClass} placeholder="Your name" />
        </Field>
...
function Field({
  ...
  return (
    <label className="grid gap-2 text-sm font-semibold">
      <span>{label}</span>
      {children}
      {error && <span className="text-xs font-medium text-gold-light">{error}</span>}
    </label>
  );
}
